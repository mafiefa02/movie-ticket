...
import { getTicketsWithTitleClient } from "@/lib/client/fetch-utils";
...
export default function ChooseSeat({
  ...
  const { data: ticket, status: ticketStatus } = useQuery<Ticket[], Error>({
    queryKey: ["tickets", movie.title],
    queryFn: () => getTicketsWithTitleClient(movie.title),
    initialData: tickets,
  });
    ...
    <Container>
      ...
      <Tabs defaultValue="10.00">
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger onClick={() => handleTimeChange("10.00")} value="10.00">
            10.00
          </TabsTrigger>
          <TabsTrigger onClick={() => handleTimeChange("12.00")} value="12.00">
            12.00
          </TabsTrigger>
          <TabsTrigger onClick={() => handleTimeChange("14.00")} value="14.00">
            14.00
          </TabsTrigger>
          <TabsTrigger onClick={() => handleTimeChange("18.00")} value="18.00">
            18.00
          </TabsTrigger>
          <TabsTrigger onClick={() => handleTimeChange("21.00")} value="21.00">
            21.00
          </TabsTrigger>
        </TabsList>
